...
describe('config', () => {
  ...
  describe('getConfig', () => {
    it('should prioritize local .env over global .commit-ai config', async () => {
      globalConfigFile = await generateConfig('.commit-ai', {
        CMT_API_KEY: 'global-key',
        CMT_MODEL: 'gpt-3.5-turbo',
        CMT_LANGUAGE: 'en'
      });

      envConfigFile = await generateConfig('.env', {
        CMT_API_KEY: 'local-key',
        CMT_LANGUAGE: 'fr'
      });
      ...
      expect(config.CMT_API_KEY).toEqual('local-key');
      expect(config.CMT_MODEL).toEqual('gpt-3.5-turbo');
      expect(config.CMT_LANGUAGE).toEqual('fr');
    });
    ...
    it('should fallback to global config when local config is not set', async () => {
      globalConfigFile = await generateConfig('.commit-ai', {
        ...
        CMT_MODEL: 'gpt-4',
        CMT_LANGUAGE: 'de',
        CMT_DESCRIPTION: 'true'
      });

      envConfigFile = await generateConfig('.env', {
        CMT_API_URL: 'local-api-url'
      });
      ...
      expect(config).not.toEqual(null);
      expect(config.CMT_API_KEY).toEqual('global-key');
      expect(config.CMT_API_URL).toEqual('local-api-url');
      expect(config.CMT_MODEL).toEqual('gpt-4');
      expect(config.CMT_LANGUAGE).toEqual('de');
      expect(config.CMT_DESCRIPTION).toEqual(true);
    });

    it('should handle boolean and numeric values correctly', async () => {
      globalConfigFile = await generateConfig('.commit-ai', {
        CMT_TOKENS_MAX_INPUT: '4096',
        CMT_TOKENS_MAX_OUTPUT: '500',
        CMT_GITPUSH: 'true'
      });

      envConfigFile = await generateConfig('.env', {
        CMT_TOKENS_MAX_INPUT: '8192',
      ...
      expect(config).not.toEqual(null);
      expect(config.CMT_TOKENS_MAX_INPUT).toEqual(8192);
      expect(config.CMT_TOKENS_MAX_OUTPUT).toEqual(500);
      expect(config.CMT_GITPUSH).toEqual(true);
      expect(config.CMT_ONE_LINE_COMMIT).toEqual(false);
    });
    ...
    it('should handle empty local config correctly', async () => {
      ...
      expect(config.CMT_API_KEY).toEqual('global-key');
      expect(config.CMT_MODEL).toEqual('gpt-4');
      expect(config.CMT_LANGUAGE).toEqual('es');
    });
    ...
    it('should override global config with null values in local .env', async () => {
      ...
      expect(config).not.toEqual(null);
      expect(config.CMT_API_KEY).toEqual(null);
    });
    ...
    it('should handle empty global config', async () => {
      ...
      expect(config).not.toEqual(null);
      expect(config.CMT_API_KEY).toEqual(undefined);
    });
  });
  ...
  describe('setConfig', () => {
    ...
    it('should create .commit-ai file with DEFAULT CONFIG if it does not exist on first setConfig run', async () => {
      const isGlobalConfigFileExist = existsSync(globalConfigFile.filePath);
      expect(isGlobalConfigFileExist).toBe(false);

      await setConfig(
        [[CONFIG_KEYS.CMT_API_KEY, 'persisted-key_1']],
        globalConfigFile.filePath
      );

      const fileContent = readFileSync(globalConfigFile.filePath, 'utf8');
      expect(fileContent).toContain('CMT_API_KEY=persisted-key_1');
    ...
    it('should set new config values', async () => {
      ...
      await setConfig(
        [
          [CONFIG_KEYS.CMT_API_KEY, 'new-key'],
          [CONFIG_KEYS.CMT_MODEL, 'gpt-4']
        ],
        globalConfigFile.filePath
      );
      ...
      expect(config.CMT_API_KEY).toEqual('new-key');
      expect(config.CMT_MODEL).toEqual('gpt-4');
    });

    it('should update existing config values', async () => {
      globalConfigFile = await generateConfig('.commit-ai', {
        CMT_API_KEY: 'initial-key'
      });
      await setConfig(
        [[CONFIG_KEYS.CMT_API_KEY, 'updated-key']],
        globalConfigFile.filePath
      );
      ...
      expect(config.CMT_API_KEY).toEqual('updated-key');
    });
    ...
    it('should handle boolean and numeric values correctly', async () => {
      ...
      await setConfig(
        [
          [CONFIG_KEYS.CMT_TOKENS_MAX_INPUT, '8192'],
          [CONFIG_KEYS.CMT_DESCRIPTION, 'true'],
          [CONFIG_KEYS.CMT_ONE_LINE_COMMIT, 'false']
        ],
        globalConfigFile.filePath
      );
    ...
    it('should persist changes to the config file', async () => {
      ...
      await setConfig(
        [[CONFIG_KEYS.CMT_API_KEY, 'persisted-key']],
        globalConfigFile.filePath
      );

      const fileContent = readFileSync(globalConfigFile.filePath, 'utf8');
      expect(fileContent).toContain('CMT_API_KEY=persisted-key');
    });
    ...
    it('should set multiple configs in a row and keep the changes', async () => {
      ...
      expect(fileContent1).toContain('CMT_API_KEY=persisted-key');
      ...
      expect(fileContent2).toContain('CMT_MODEL=gpt-4');
    });
  });
});
