...
it('cli flow to generate commit message for 1 new file (staged)', async () => {
  ...
  const { queryByText, findByText, userEvent } = await render(`CMT_AI_PROVIDER='test' node`, [resolve('./out/cli.cjs')], { cwd: gitDir });
...
it('cli flow to generate commit message for 1 changed file (not staged)', async () => {
  ...
  const { findByText, userEvent } = await render(`CMT_AI_PROVIDER='test' node`, [resolve('./out/cli.cjs')], { cwd: gitDir });
