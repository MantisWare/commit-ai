...
export const hookCommand = command(
  ...
  async (argv) => {
    ...
    try {
      ...
      if (mode === 'set') {
        intro(`setting commit-ai as '${HOOK_NAME}' hook at ${SYMLINK_URL}`);
        ...
        if (await isHookExists()) {
          ...
          if (realPath === HOOK_URL)
            return outro(`CommitAI is already set as '${HOOK_NAME}'`);
      ...
      if (mode === 'unset') {
        ...
        if (!(await isHookExists())) {
          return outro(
            `CommitAI wasn't previously set as '${HOOK_NAME}' hook, nothing to remove`
          );
        }
        ...
        if (realpath !== HOOK_URL) {
          return outro(
            `CommitAI wasn't previously set as '${HOOK_NAME}' hook, but different hook was, if you want to remove it — do it manually`
          );
        }
      ...
      throw new Error(
        `Unsupported mode: ${mode}. Supported modes are: 'set' or 'unset'. Run: \`cmt hook set\``
      );
    } catch (error) {
