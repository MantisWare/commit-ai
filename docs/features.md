...
- **Maximum files**: When `CMT_MAX_FILES` is set, `cmt` automatically splits oversized staged changes into multiple commits (messages generated in parallel via `CMT_CHUNK_CONCURRENCY`, one confirmation for all). Git hooks still require staging within the limit.
